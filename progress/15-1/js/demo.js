...
window.onload = function () {
	...
	$('nav > a:first-of-type').on('click', function(event) {
		...
		// Get all the non-first <a> elements under <nav> and store them into menuItems.
		let menuItems = $('nav > a:not(:first-of-type)');
		// Set up a varibale for creating a different length of animation delay
		let delay = 0;
		if(menuOpen) {
			menuItems.each(function(index) {
				setTimeout(function() {
		...
		// When menuOpen is false, try to show the entire menu.
			...
			// Run a function with the index information
			//every items stored in menuItems
			menuItems.each(function(index) {
